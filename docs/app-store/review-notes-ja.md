...
初回の外部AI送信前に、Sonioxへ動画ファイル全体（映像・音声）、Alibaba Cloud Qwenへ文字起こし・映像解析テキスト・撮影日時を送る目的を説明し、明示的なAI同意を求めます。同意しなくても撮影、端末上のHEVC最適化、非公開タイムライン、再生、個別削除、アカウント削除を利用できます。同意すると既存・新規の動画がMCP/AIエージェントの読み取り対象になりますが、動画ごとに共有をオフにできます。同意撤回後は新規の文字起こし、解析、要約、MCP取得を停止し、既存動画もMCPから取得できなくなります。
