...
初回の外部AI送信前に、Sonioxへ動画音声、Alibaba Cloud Qwenへframe・文字起こし・解析内容を送る目的を説明し、明示的なAI同意を求めます。同意しなくても撮影、端末上のHEVC最適化、非公開タイムライン、再生、個別削除、アカウント削除を利用できます。同意撤回後は新規の文字起こし、解析、要約、MCP取得を停止します。AIエージェントにはユーザーが個別に許可した動画だけを読み取り可能にします。
